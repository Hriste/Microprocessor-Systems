...
void main (void)
{
  ...
  //  printf("Ground /INT0 on P0.2 to generate an interrupt.\n\n\r");
    ...
    while (1)                   // No need to set UART0_PAGE
    {
		...
		if (time%10==0&& PRINTED==0){printf("%d tenths of a second have passed \n\r",time);PRINTED =1;}
    }
}
//-------------------------------------------------------------------------------------------
...
void UART0_INIT(void)
{
    ...
    TMOD   &= ~0xF0;
    TMOD   |=  0x21;            // Timer1, Mode 2: 8-bit counter/timer with auto-reload.
    TH1     = (unsigned char)-(SYSCLK/BAUDRATE/16); // Set Timer1 reload value for baudrate
    CKCON  |= 0x18;             // Timer1 uses SYSCLK as time base.
    TL1     = TH1;
    TR1     = 1;                // Start Timer1.
	TL0 = TH0;
	TR0 = 1;
