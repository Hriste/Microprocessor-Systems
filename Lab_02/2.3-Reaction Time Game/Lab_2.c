...
__bit PRINTED = 0;//flag for if we've already printed the second has passed
__bit SW2press = 0;
//-------------------------------------------------------------------------------------------
// Function PROTOTYPES
//-------------------------------------------------------------------------------------------
...
void main (void)
{
	...
	printf("When the screen flashes hit the button\n\r")
    ...
    while (1)                   // No need to set UART0_PAGE
    {
		...
		if(tenth==rand(){
			...
			turns = turns +1;
			printf("/033[5,1H");
			printf("Your average response time is %d",slowness/turns;}
    }
}
//-------------------------------------------------------------------------------------------
...
int flash(void)
{
	...
	printf("/033[47; 30m");//turn screen white and text black
	
	while(time<2);// wait .2 seconds
	
	printf("/033[40; 37m")//turn screen black and text white
	
	while(SWpress==0);
	reaction = time;
	printf("/033[4,1H")
	printf("You reacted in %d tenths of a second", reaction)
	SWpress = 0;
	tenth = 0;
	time = 0;
	TL0 = TH0 = 0; // reset timer
